...
\subsection{Estimateur de mu dans un cas simple} % (fold)
\label{sub:estimateur_de_mu_dans_un_cas_simple}


On va mettre de coté les paramères de nuisance dans ce problème.
Même si on va quand même leur affecter des symboles.

Notation :
\begin{itemize}
	\item $\alpha$ = paramètre de nuisance
	\item $\beta$ = paramètre de la loi de Poisson des bruits de fond
	\item $\gamma$ = paramètre de la loi de Poisson des signaux
	\item $n$ = le nombre d'event
	\item $s$ = nombre de signaux
	\item $b$ = nombre de bruit de fond
	\item $\mu$ = déviation de l'espérance du nombre de signaux
\end{itemize}
