...
Let's start with the obvious quantities :
\begin{equation}
	n = n \times 1 = n \times \int_{x \in \Omega_{tot}} p(x) dx
\end{equation}
The number of events in a bin is simply the fraction of events that falls inside a bin.
An event fall inside the i-th bin with probability $p(x \in \Omega_i)$ defined as:
\begin{equation}
	p(x \in \Omega_i) = \int_{x \in \Omega_i} p(x) dx
\end{equation}
\begin{equation}
	\label{eq:nb_events_in_bin}
	n_i = n \times p(x \in \Omega_i)  = n \times \int_{x \in \Omega_i} p(x) dx
\end{equation}
The same applies to $s$ and $b$.
But let's keep pushing open doors for clarity.
An event is either a signal event $S$ or a background event $B$.
Therefor we can decompose the density into two parts :
\begin{equation}
	p(x) = p(x|S) p(S) + p(x|B) p(B)
\end{equation}
Injecting this into \autoref{eq:nb_events_in_bin}
\begin{align}
	n_i &= n \times \int_{x \in \Omega_i} p(x) dx \\
	n_i &= n \times \int_{x \in \Omega_i} p(x|S) p(S) + p(x|B) p(B) dx \\
	n_i &= n \times \left ( \int_{x \in \Omega_i} p(x|S) p(S) dx + \int_{x \in \Omega_i}  p(x|B) p(B) dx \right )\\
	n_i &= n \times \int_{x \in \Omega_i} p(x|S) p(S) dx + n \times \int_{x \in \Omega_i}  p(x|B) p(B) dx \\
	n_i &= n \times p(S) \int_{x \in \Omega_i} p(x|S) dx + n \times p(B) \int_{x \in \Omega_i}  p(x|B) p(B) dx \\
	n_i &= s \int_{x \in \Omega_i} p(x|S) dx + b \int_{x \in \Omega_i}  p(x|B) p(B) dx \\
\end{align}
We find that $s = n \times p(S)$ and $b = n \times p(B)$ which is again obvious statement since the number of signal events is the total number of event times the probability of an event to be a signal.
\begin{equation}
	s_i = s \times \int_{x \in \Omega_i} p(x|S) p(S) dx
\end{equation}
\begin{equation}
	b_i = b \times \int_{x \in \Omega_i} p(x|B) p(B) dx
\end{equation}

See where is the mistake here ? 

We should be using expected number everywhere !!!!
